/* Description: Calculate the correlation between ETH price and NFT index price over time. */
...
SELECT 
    ...
    eth_price,
    nft_index_price,
    CORR(eth_price, nft_index_price) OVER (ORDER BY day) AS "Running Pearson Correlation",
    CORR(eth_price, nft_index_price) OVER (ORDER BY day ROWS BETWEEN 14 PRECEDING AND CURRENT ROW) AS "Running Pearson Correlation 14 days",
    CORR(eth_price, nft_index_price) OVER (ORDER BY day ROWS BETWEEN 30 PRECEDING AND CURRENT ROW) AS "Running Pearson Correlation 30 days"
FROM 
    prices
ORDER BY 1 DESC;