...
WITH 
  ...
  t AS (
    ...
    FROM nft.trades n
    WHERE (block_time > now() - interval  '12 MONTHS') AND 
    number_of_items = 1 
  ...
  pnl AS (
    SELECT 
      ...
      "ATH Floor Price (ETH)",
      "ATH Floor Price (USD)",
      -- (c."Current Floor Price (ETH)" - m."ATH Floor Price (ETH)") / m."ATH Floor Price (ETH)"::FLOAT AS profit_eth,
...
SELECT
  count_profit_usd,
  count_total,
  count_profit_usd/count_total::FLOAT AS profit_share_usd
FROM
(
  SELECT
