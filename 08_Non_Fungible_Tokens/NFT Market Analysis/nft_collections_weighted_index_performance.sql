/* Description: Calculate weighted index for top 10 and next 25 NFT medians. */
WITH 
  t1 AS (
    SELECT 
    ...
    WHERE 
      ...
      AND date_trunc('day', block_time ) > date_trunc('day', now()) - INTERVAL '24 WEEKS'
      ...
      AND nft.trades.tx_hash NOT IN (
        SELECT 
          tx_hash
        FROM 
          nft.trades
        WHERE 
          "tx_from" = '0x0000000000000000000000000000000000000000'
      )
      GROUP BY 
        1, 2
  ),
...
SELECT 
  day,
  -- top_10_index_price,
  -- FIRST_VALUE(top_10_index_price) OVER (ORDER BY day ASC),
  top_10_index_price/FIRST_VALUE(top_10_index_price) OVER (ORDER BY day ASC)*100 AS weighted_index_top_10,
  -- next_25_index_price,
  -- FIRST_VALUE(next_25_index_price) OVER (ORDER BY day ASC),
  next_25_index_price/FIRST_VALUE(next_25_index_price) OVER (ORDER BY day ASC)*100 AS weighted_index_next_25
FROM (
  SELECT 
    COALESCE(t1.day, t2.day) AS day,
    SUM(top_10_nft_median) AS top_10_index_price,
    SUM(nft_next_25_median) AS next_25_index_price
  FROM 
    t1
    FULL JOIN t2 ON t2.day = t1.day
  GROUP BY 
    1
) x
ORDER BY 
