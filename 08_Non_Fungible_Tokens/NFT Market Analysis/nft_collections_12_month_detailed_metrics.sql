/* Description: Calculate NFT floor prices and volume metrics. */
...
t AS (
    SELECT
        ...
        CASE 
            WHEN nftp."nft_collection" IS NOT NULL THEN CONCAT('<a href="',nftp.url,'" target="_blank">',nftp.nft_collection,'</a>') 
            ELSE CONCAT('<a href="https://etherscan.io/token/0',SUBSTRING("nft_contract_address"::text, 2),'">',CONCAT('0',SUBSTRING("nft_contract_address"::text, 2)),'</a>') 
        END AS "OPENSEA",
    ...
    FROM nft.trades n LEFT JOIN dune_user_generated."cryptuschrist_nft_projects2" nftp 
    ON nftp."nft_contract" = n.nft_contract_address
    ...
    GROUP BY 1, nft_contract_address, nftp.nft_collection, nftp.url
    -- HAVING SUM(original_amount) >= 3000
),
...
SELECT 
    ...
    c."Current Floor Price (USD)" ,
    m."ATH Floor Price (USD)",
    (c."Current Floor Price (ETH)" - m."ATH Floor Price (ETH)") / m."ATH Floor Price (ETH)" * 100 AS "% From ATH (ETH)",
    (c."Current Floor Price (USD)" - m."ATH Floor Price (USD)") / m."ATH Floor Price (USD)" * 100 AS "% From ATH (USD)",
