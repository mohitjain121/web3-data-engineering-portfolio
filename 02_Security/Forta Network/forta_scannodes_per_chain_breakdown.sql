/*
Description: This query calculates the number of nodes that meet the minimum stake requirement for each day and chain.
*/
...
WITH 
    ...
    tmp AS (
        SELECT
            ...
            s.day, 
            CASE 
                WHEN CAST(chainId AS integer) = 1 THEN 'ethereum'
                WHEN CAST(chainId AS integer) = 137 THEN 'polygon'
                WHEN CAST(chainId AS integer) = 43114 THEN 'avalanche'
                WHEN CAST(chainId AS integer) = 56 THEN 'bsc'
                WHEN CAST(chainId AS integer) = 10 THEN 'optimism'
                WHEN CAST(chainId AS integer) = 250 THEN 'fantom'
                WHEN CAST(chainId AS integer) = 42161 THEN 'arbitrum'
                ELSE chainId
            END AS chain
        FROM 
            stake_node_id s
