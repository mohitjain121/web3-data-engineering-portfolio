/*
Description: This query calculates the number of active bots on different chains.
*/
...
(
    SELECT 
        df.day,
        CASE 
            WHEN CAST(s.chains AS INTEGER) = 1 THEN 'ethereum'
            WHEN CAST(s.chains AS INTEGER) = 137 THEN 'polygon'
            WHEN CAST(s.chains AS INTEGER) = 43114 THEN 'avalanche'
            WHEN CAST(s.chains AS INTEGER) = 56 THEN 'bsc'
            WHEN CAST(s.chains AS INTEGER) = 10 THEN 'optimism'
            WHEN CAST(s.chains AS INTEGER) = 250 THEN 'fantom'
            WHEN CAST(s.chains AS INTEGER) = 42161 THEN 'arbitrum'
        END AS chain,
        COUNT(df.agentid) + 8 AS n_of_bots
    FROM 
        daily_filled df
        INNER JOIN 
        stake_node_id s
