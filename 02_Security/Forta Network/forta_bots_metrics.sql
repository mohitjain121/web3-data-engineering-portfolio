/*
Description: This query calculates the number of active bots per day, considering only agents with a minimum stake of 100.
*/
...
daily_latest_tmp AS (
    SELECT 
        ...
        agentid,
        value,
        ROW_NUMBER() OVER (PARTITION BY agentid, day ORDER BY day DESC, block_number DESC) AS filter
    FROM 
        events
),
...
(
    SELECT 
        df.day,
        COUNT(df.agentid) + 55 AS n_of_bots
    FROM 
        daily_filled df
        INNER JOIN 
        minimum_stake s
