WITH
  ...
  generateDummyData AS (
    ...
    FROM
      txns
      CROSS JOIN generate_series('2021-12-23', NOW(), '1 WEEK') as "generated_date"
  )
select
  "datex",
  "vault",
  vault_name,
  "contract_address",
  "token_tvl" / 10 ^ "decimals" as "daily_amount",
  SUM("token_tvl" / 10 ^ "decimals") OVER (
    PARTITION BY "vault",
    "contract_address"
    ORDER BY
      "datex"
