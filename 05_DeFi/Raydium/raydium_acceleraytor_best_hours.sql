/* Description: Calculate average and median ratio filled for accelerator projects at start and end times. */

WITH 
  acceleraytor_shini AS (
  VALUES 
...
SELECT 
  COALESCE(
    EXTRACT(HOUR FROM TIMESTAMP 'epoch' + a.start_time * INTERVAL '1 second'), 
    EXTRACT(HOUR FROM TIMESTAMP 'epoch' + b.end_time * INTERVAL '1 second')
  ) AS hour,
  AVG(a.ratio_filled) AS avg_ratio_filled_start,
  PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY a.ratio_filled) AS med_ratio_filled_start,
  AVG(b.ratio_filled) AS avg_ratio_filled_end,
  PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY b.ratio_filled) AS med_ratio_filled_end
FROM 
  acceleraytor_shini a
  FULL JOIN acceleraytor_shini b ON EXTRACT(HOUR FROM TIMESTAMP 'epoch' + a.start_time * INTERVAL '1 second') = EXTRACT(HOUR FROM TIMESTAMP 'epoch' + b.end_time * INTERVAL '1 second')
GROUP BY 1;