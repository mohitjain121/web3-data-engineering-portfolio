/* Description: Calculate incentives and farm statistics. */
...
WITH 
  ...
  incentives AS (
    SELECT 
      token_pair_name,
      COUNT(reward_mint) AS count_incentives
    FROM 
      token_pair_names
    GROUP BY 
      1
...
SELECT 
  count_incentives,
  COUNT(token_pair_name) AS number_of_farms,
  array_join(collect_list(token_pair_name), '   |   ') AS pair_list
FROM 
  incentives
GROUP BY 
  1
