/* Description: 
  This query categorizes Solana transactions based on program types and calculates the percentage of users for each category.
*/
...
program_types AS (
  SELECT 
    CASE 
      ...
      OR array_contains(account_keys, '9W959DqEETiGZocYWCQPaJ6sBmUzgfxXfqGeTEdp3aQP') THEN 'Mango'
...
users AS (
  ...
  FROM 
    solana.transactions, total_users
  WHERE 
    account_keys[0] IN (SELECT account_keys[0] FROM solana.transactions
    WHERE array_contains(account_keys, 'FarmqiPv5eAj3j1GMdMCMUGXqPUvmquZtMy86QH6rzhG')
    AND success == true
