/* Description: Calculate total dollar rewards for each token pair. */
...
SELECT 
    ...
    base_token, 
    quote_token,
    array_join(collect_list(reward_token), '   |   ') AS reward_token_list,
    FROM_UNIXTIME(reward_open_time, 'yyyy-MM-dd HH:mm') AS reward_open_time,
    FROM_UNIXTIME(reward_end_time, 'yyyy-MM-dd HH:mm') AS reward_end_time,
    (reward_end_time - reward_open_time) / 86400::FLOAT AS days_opened,
    SUM((reward_end_time - reward_open_time) * price * reward_per_second / POWER(10, token_decimals)) AS total_dollar_rewards,
    amm_id
FROM 
    token_pair_names t
