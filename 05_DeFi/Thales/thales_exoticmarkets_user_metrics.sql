/*
Description: Analyze target wallets' interactions with Thales and Exotic Markets.
*/
...
WITH 
  ...
  exotic_markets_participant AS (
    SELECT 
      date_trunc('day', evt_block_time) AS day,
      "account" AS address
    FROM 
      exotic_markets."ExoticPositionalFixedMarket_evt_NewPositionTaken"
  ),
  exotic_markets_withdrawn AS (
    SELECT 
      date_trunc('day', evt_block_time) AS day,
      "account" AS address
    FROM 
      exotic_markets."ExoticPositionalFixedMarket_evt_TicketWithdrawn"
  ),
  transfers AS (
    SELECT 
      day,
    ...
    FROM 
      (
        ...
        WHERE 
          contract_address = '\x217D47011b23BB961eB6D93cA9945B7501a5BB11'
        UNION ALL
        SELECT 
          date_trunc('day', evt_block_time) AS day,
          "from" AS address,
  ...
  ever_held_thales AS (
    SELECT 
      COUNT(DISTINCT address) AS ever_held_thales
    FROM 
      balance_all_days
    WHERE 
      day < '2022-05-30'
  ...
  used_exotic_markets AS (
    SELECT 
      COUNT(DISTINCT address) AS used_exotic_markets
    FROM 
      exotic_markets_participant
    WHERE 
      day < '2022-05-30'
      AND address IN (
        SELECT 
  ...
  held_exotic_markets_position AS (
    SELECT 
      (SELECT COUNT(*) FROM target_wallets) - COUNT(DISTINCT address) AS held_exotic_markets_position
    FROM 
      exotic_markets_withdrawn
    WHERE 
      day >= '2022-05-30'
      AND day < '2022-06-06'
      AND address IN (
  ...
  used_exotic_markets_after_bounty AS (
    SELECT 
      COUNT(DISTINCT address) AS used_exotic_markets_after_bounty
    FROM 
      exotic_markets_participant
    WHERE 
      day > '2022-05-31'
...
SELECT 
  ...
  held_before_bounty,
  used_exotic_markets,
  held_exotic_markets_position,
  used_exotic_markets_after_bounty
FROM 
